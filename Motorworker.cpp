...
void Motorworker::ReadState(int moteus_id, State& curr_state) const {
    ...
    q_com.mode = mjbots::moteus::Resolution::kInt16;
...
void Motorworker::getFromMain_motor_commands(QString msg, int Motor_id) // slot implementation
{
    ...
    else if (msg == "Check Device")
    {
        ...
        moteus::Controller::ProcessTransportArgs({"COM5"});
        moteus::Controller::Options options;
        options.id = Motor_id;
        options.default_query = false;
        moteus::Controller controller(options);
...
//        }
    ...
    else if (msg == "Get Cur X,Y")
    {
        ...
        if (Collision_Check_enable)
        {
            ...
            // convert revolutions to radians
            double theta1 = -motor1_revolutions * 2 * M_PI;
            double theta2 = motor2_revolutions * 2 * M_PI;
...
//        }
            ...
            else
            {
                ...
                double motor1_revolutions1 = theta[0]/(2*M_PI);
                double motor2_revolutions1 = theta[1]/(2*M_PI);
                double motor1_revolutions2 = theta[2]/(2*M_PI);
                double motor2_revolutions2 = theta[3]/(2*M_PI);
