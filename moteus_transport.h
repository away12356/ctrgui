...
class Fdcanusb : public Transport {
 public:
  ...
  Fdcanusb(HANDLE read_fd, HANDLE write_fd, const Options& options = {})
      : options_(options) {
...
//  }
    ...
    static int64_t GetNow() {
        ...
        int64_t x=(counter.QuadPart * 1000000000ll) / frequency.QuadPart;
        // 计算纳秒数 = (计数器值 * 10^9) / 频率
        return x;
    }
 ...
 private:
  void Open(const std::string& device_in) {
    ...
    if (fd == INVALID_HANDLE_VALUE) {
        ...
        qDebug()<< "打开串口失败! 错误代码: 0x" << err << endl;
    }
    else
    {
        qDebug()<< "打开串口成功!";
  ...
  void Open(HANDLE read_fd, HANDLE write_fd) {
    read_fd_ = read_fd;
    write_fd_ = write_fd;
...
//      tmo.tv_nsec = to_sleep_ns % 1000000000;
      ...
      ClearCommError(read_fd_, &dwErrors, &comStat);
      if (comStat.cbInQue > 0) {
          int n=(ReadFile(read_fd_, &line_buffer_[line_buffer_pos_], comStat.cbInQue, &bytesRead, NULL));
          QByteArray data = QByteArray::fromRawData(line_buffer_, bytesRead);
          line_buffer_pos_ += bytesRead;
          qDebug()<<"接收到的数据"<<QByteArray(data)<<line_buffer_pos_;
      }
      line_buffer_pos_ = bytesRead;
...
//      }
  ...
  ConsumeCount CHILD_ConsumeLines(std::vector<CanFdFrame>* replies) {
    ...
    //qDebug()<<"rcv"<<result.rcv<<"ok"<<result.ok;
    return result;
  }
  ...
  void CHILD_SendCanFdFrame(const CanFdFrame& frame) {
    ...
    for (size_t n = 0; n < p.size(); ) {
      if (!WriteFile(write_fd_, buf, p.size() - n, &bytesWritten, nullptr)) {
          qDebug()<< "发送数据失败";
      }
      else{
          qDebug()<<"发送的数据"<<QByteArray::fromRawData(buf, bytesWritten);
          n += bytesWritten;
      }
    }
  }
